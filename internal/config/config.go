...
type Config struct {
	...
	MultipartCleanupInterval, MultipartCleanupMaxAge                                                                                                                                                                                   time.Duration
	AuditForwarderBatchSize                                                                                                                                                                                                            int
	AuditEnabled, OIDCLogoutEnabled, MultipartCleanupEnabled                                                                                                                                                                           bool
	OIDCDebugJWT                                                                                                                                                                                                                       bool
...
func Load() Config {
	...
	cfg := Config{Addr: value("ADDR", ":8080"), DataDir: value("DATA_DIR", "./data"), DatabaseDriver: databaseDriver, DatabaseURL: databaseURL, AuditAdminGroup: value("AUDIT_ADMIN_GROUP", "admins"), OrganizationEncryptionKey: os.Getenv("MARINER_ORG_ENCRYPTION_KEY"), AuditForwarderPollingInterval: durationValue("AUDIT_FORWARDER_POLLING_INTERVAL", 2*time.Second), SessionIdleTimeout: durationValue("SESSION_IDLE_TIMEOUT", 30*time.Minute), MultipartCleanupInterval: durationValue("MULTIPART_CLEANUP_INTERVAL", 15*time.Minute), MultipartCleanupMaxAge: durationValue("MULTIPART_CLEANUP_MAX_AGE", 12*time.Hour), AuditForwarderBatchSize: valueInt("AUDIT_FORWARDER_BATCH_SIZE", 100), AuditEnabled: os.Getenv("AUDIT_ENABLED") != "false", OIDCLogoutEnabled: os.Getenv("OIDC_LOGOUT_ENABLED") != "false", MultipartCleanupEnabled: os.Getenv("MULTIPART_CLEANUP_ENABLED") != "false", OIDCIssuer: os.Getenv("OIDC_ISSUER"), OIDCClientID: os.Getenv("OIDC_CLIENT_ID"), OIDCClientSecret: os.Getenv("OIDC_CLIENT_SECRET"), OIDCRedirectURL: os.Getenv("OIDC_REDIRECT_URL"), CookieSecret: value("COOKIE_SECRET", "change-me-in-production"), OIDCGroupsClaim: value("OIDC_GROUPS_CLAIM", "groups"), OIDCAudienceClaim: value("OIDC_AUDIENCE_CLAIM", "aud"), OIDCAudience: value("OIDC_AUDIENCE", os.Getenv("OIDC_CLIENT_ID")), OIDCNameClaim: value("OIDC_NAME_CLAIM", "name"), OIDCScopes: strings.Fields(value("OIDC_SCOPES", "openid profile email")), OIDCDebugJWT: os.Getenv("OIDC_DEBUG_JWT") == "true"}
